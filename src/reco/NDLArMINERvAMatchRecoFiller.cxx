...
namespace cafmaker
{
 ...
 // match tracks using the info that should have been filled by the ND-LAr and MINERvA reco fille$
   ...
   // Loop over the minerva tracks
     ...
     To define matching criteria it is needed to find angleas between LAr and MINERvA tracks. 
     ...
     the plane (parallel to plane XY) of LAr detector.
       */
			...
			theta_mn_x = atan(tg_theta_mn_x);
                        ...
                        double t_mn; //parametr of the equation of the line (minerva track)
...
costheta=((x2_minerva-x1_minerva)*(x2_lar-x1_lar)+(y2_minerva-y1_minerva)*(y2_lar-y1_lar)+(z2_minerva-z1_minerva)*(z2_lar-z1_lar))/(len_nd*len_mn); //angle between minerva and Lar tracks
     ...
     if (abs(delta_theta_x) < 0.04 && abs(delta_theta_y) < 0.05 && abs(dist_x) < 7 && abs(dist_y) < 7 && abs(len_nd) >50) {
